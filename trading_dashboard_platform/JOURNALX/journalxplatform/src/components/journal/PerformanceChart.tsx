...
export function PerformanceChart({ 
  ...
  // Determine the glow class based on performance
  const glowClass = `glow-${overallPerformance}`;
      
  return (
    <ResponsiveContainer width="100%" height="100%" className={glowClass}>
      <LineChart
        data={data}
      ...
      >        
              ...
              return (
                <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'} border rounded p-3 shadow-lg`}>
                  ...
                  <p className={`text-sm ${Number(payload[0].value) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    Daily: {`${valuePrefix}${Number(payload[0].payload.value).toLocaleString('en-US', {minimumFractionDigits: 2})}${valueSuffix}`}
                  </p>
                  <p className={`text-sm font-medium ${Number(payload[0].payload.cumulativeValue) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <Line
          ...
          name={showCumulative ? "Cumulative P&L" : "Daily P&L"}
          stroke={lineColor}
          strokeWidth={3}
