...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const transformUser = (supabaseUser: SupabaseUser, profile?: any): User => {
    return {
      ...
      emailVerified: profile?.email_verified || supabaseUser.email_confirmed_at !== null || false,
    };
  };
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes('User already registered')) {
          ...
          toast.error('Too many signup attempts. Please wait 15-60 minutes and try again, or use a different email address for testing.');
        } else if (error.message?.includes('For security purposes, you can only request this after')) {
          const match = error.message.match(/after (\d+) seconds/);
          const seconds = match ? match[1] : '60';
          toast.error(`Rate limit reached. Please wait ${Math.ceil(parseInt(seconds) / 60)} minutes before trying again.`);
        } else if (error.message?.includes('Invalid email')) {
          toast.error('Please enter a valid email address.');
        } else if (error.message?.includes('Password should be at least')) {
          toast.error('Password must be at least 6 characters long.');
        } else if (error.message?.includes('Error sending confirmation email')) {
          toast.error('Unable to send confirmation email. Please check your email configuration in Supabase Dashboard.');
        } else {
          toast.error(handleSupabaseError(error));
        }
        return false;
